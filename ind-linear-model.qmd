...
```{r}
sse_set1 <- (y - X %*% beta_set1)^2 |>
  sum()
sse_set2 <- (y - X %*% beta_set2)^2 |>
  sum()
sse_set3 <- (y - X %*% beta_set3)^2 |>
  sum()
sse_set4 <- (y - X %*% beta_set4)^2 |>
  sum()
...
```{r}
lmod <- splm(log_cond ~ temp, data = lake, spcov_type = "none")
coef(lmod)
```
